...
// ── UI/UX Pro Max Skill 自动安装 ──────────────────────

function installUIUXSkill(targetDir, uiSkillDir) {
  ...
  try {
    ...
    fileLog(SYM.warn, `跳过 ${DIM}(未检测到 npm)${RESET}`);
    console.log(`        ${DIM}手动: npm i -g uipro-cli && cd ${targetDir} && uipro init --ai claude${RESET}`);
    return false;
  }
  ...
  try {
    ...
    console.log(`        ${DIM}安装 uipro-cli...${RESET}`);
    try {
      execSync('npm install -g uipro-cli', { stdio: 'ignore', timeout: 60000 });
    } catch (e) {
      fileLog(SYM.warn, `CLI 安装失败 ${DIM}(权限不足？)${RESET}`);
      console.log(`        ${DIM}手动: sudo npm i -g uipro-cli && cd ${targetDir} && uipro init --ai claude${RESET}`);
      return false;
    }
  }

  // 运行 uipro init
  console.log(`        ${DIM}安装 UI/UX Skill...${RESET}`);
  try {
    execSync(`cd "${targetDir}" && uipro init --ai claude`, { stdio: 'ignore', timeout: 60000 });
    if (fs.existsSync(uiSkillDir)) {
      fileLog(SYM.check, 'UI/UX Pro Max Skill');
      return true;
    }
  } catch (e) {
    // 静默失败
  ...
  fileLog(SYM.warn, `安装失败`);
  console.log(`        ${DIM}手动: cd ${targetDir} && uipro init --ai claude${RESET}`);
  return false;
}
...
function install(targetDir) {
  ...
  // v1.0.0~v1.0.6 迁移：旧版状态内嵌在 CLAUDE.md 的 YAML 块中，提取后写入 project-state.md
  let legacyYamlBlock = null;
  if (isUpgrade) {
    ...
    if (!fs.existsSync(targetState)) {
      ...
      if (yamlMatch && /current_phase:\s*P[1-6]/.test(yamlMatch[0])) {
        legacyYamlBlock = yamlMatch[1].trim();
      }
    }
  }
  ...
  // 升级清理：删除 v1.4.x 及以前的 commands/ 目录（已被 skills/ 取代）
  const legacyCommandsDir = path.join(targetDir, '.claude', 'commands');
  if (fs.existsSync(legacyCommandsDir)) {
    fs.rmSync(legacyCommandsDir, { recursive: true, force: true });
    fileLog(SYM.check, `.claude/commands/ ${DIM}(旧版已删除，已被 skills/ 取代)${RESET}`);
  }
  ...
  } else if (legacyYamlBlock) {
    // v1.0.0~v1.0.6 迁移：将旧 CLAUDE.md 中提取的状态写入新的 project-state.md
    const stateTemplate = fs.readFileSync(templateState, 'utf-8');
    const migrated = stateTemplate.replace(
      /```yaml\n[\s\S]*?```/,
      '```yaml\n' + legacyYamlBlock + '\n```'
    );
  ...
  // Step 9: 自动安装 UI/UX Pro Max Skill
  blank();
  step(9, 9, '安装 UI/UX Pro Max Skill');
  const uiSkillDir = path.join(targetDir, '.claude', 'skills', 'ui-ux-pro-max');

  if (fs.existsSync(uiSkillDir)) {
    fileLog(SYM.check, `UI/UX Pro Max Skill ${DIM}(已安装)${RESET}`);
  } else {
    const installed = installUIUXSkill(targetDir, uiSkillDir);
    if (installed) {
      skillCount++;
    }
  }
