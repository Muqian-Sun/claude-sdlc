...
P1.2 技术调研 → Context7 + WebSearch 查最新方案
  ↓
P1.3 编写 PRD →【你确认】✓ 第一个检查点
  ↓
P1.4 架构设计 → 模块划分 + 数据模型
  ↓
P1.5 原型设计 → Chrome 展示（如涉及 UI）→【你确认】✓ 第二个检查点
  ↓ 以下全自动
P2 编码 → P3 测试 → P4 综合审查（唯一审查）→ P5 交付 → 完成报告
```

**用户只需两次确认（PRD 确认 + 设计确认），其余全自动。** P4 是唯一正式审查关卡（代码质量+测试质量+集成一致性+PRD追溯），未通过自动修复重试，最多 3 次后才请求帮助。

**关键改进**：P1 增加需求澄清环节，通过多轮对话充分了解需求，避免基于假设开发。
