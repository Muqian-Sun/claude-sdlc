...
- P4 阶段开始时，先列出 PRD→测试映射表
- 编写测试时逐条覆盖 PRD 需求，确保无遗漏
- 不为 PRD 范围外的代码编写测试（如果存在 PRD 外代码，应回退到 P3 删除）
- 测试的验收标准以 PRD 中定义的验收标准为准
...
每次 `npx vitest run` / `npx jest` / `pytest` 都有编译+启动开销（通常 5-15 秒）。反复运行同一测试文件会浪费大量时间。

### 正确流程
1. 运行一次测试，用 `tee /tmp/sdlc-test-output.txt` 保存完整输出
2. 用 Read 工具读取 `/tmp/sdlc-test-output.txt` 分析所有结果
3. 如有失败，从输出文件一次性收集所有错误信息，**批量修复**所有问题
4. 修复完成后再运行一次验证
5. **整个 P4 阶段最多运行 3 次测试命令**

### 禁止行为
- ❌ 对同一测试文件/套件反复执行测试命令
- ❌ 用 `-t "test name"` 单独跑某个测试用例（启动开销与跑全部一样大）
- ❌ 每修复一个失败就重跑一次测试
- ❌ 用不同的 `grep` 过滤条件重新运行测试来查看不同信息
