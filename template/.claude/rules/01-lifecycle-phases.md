...
**审查策略**：**P5 是唯一正式审查关卡**（代码+测试+集成+PRD追溯）。P1 靠用户确认推进，P3/P4/P6 完成后直接推进。

---

## P1 — 需求分析 + 架构设计

分析用户需求，检查 `project_roadmap` 和 `completed_tasks` 确保与整体规划一致。**先调研，再设计架构，涉及 UI 时创建原型在 Chrome 展示**，最后输出包含架构方案的完整 PRD，**用户确认后直接推进到 P3 并启动自动驱动**。
