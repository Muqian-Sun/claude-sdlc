...
1. 读取当前阶段，检查完成条件
2. **P1（用户确认 PRD 即推进）**：
   - 用户已确认 PRD（含架构+原型）→ 直接更新 project-state.md
   - P1→P2 提示"进入自动驱动模式"
3. **P2/P3/P5（完成即推进，不审查）**：
   - P2：PRD 全实现 + 代码可编译 → 直接进 P3
   - P3：测试全通过 + 覆盖率达标 → 直接进 P4
   - P5：交付完成 → 输出交付摘要报告
4. **P4（唯一正式审查）**：
...
P1 确认 PRD 后，P2-P5 自动驱动：编码→测试→P4综合审查→交付→完成。P4 是唯一审查关卡，未通过自动修复重试（最多3次）。
