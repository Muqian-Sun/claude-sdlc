...
**测试只跑一次，输出保存到文件，从文件分析结果。** 禁止对同一测试反复执行。

1. `npx vitest run --coverage 2>&1 | tee /tmp/sdlc-test-output.txt; echo "TEST_EXIT=${PIPESTATUS[0]}"`
2. 用 Read 工具读取 `/tmp/sdlc-test-output.txt` 分析结果（通过/失败/覆盖率）
3. 如有失败 → 从输出文件**一次性收集所有失败信息** → **批量修复** → 再跑一次验证
4. 整个 P4 审查**最多运行 3 次测试命令**
5. 禁止：用 `-t "test name"` 单独跑某个用例、每修一个跑一次、用不同 grep 重跑来看不同信息
